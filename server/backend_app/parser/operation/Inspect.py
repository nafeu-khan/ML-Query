...
def inspect(command):
    ...
    if operation_type.upper()=="CHECKNULL":
        response['text']= checknull(url,dataset_name)
        return response
    elif operation_type.upper() =="ENCODING":
        res=encoding(url,dataset_name,command_parts)
        if res: return res
        else:
            response['text']="Something wrong. try again"
            return response
    elif operation_type.upper() =="DEDUPLICATE":
        res= deduplicate(url,command_parts)
        if res:
            return res
        else:
            response['text']="Something wrong. try again"
        ...
        res = categorize(url,dataset_name,command_parts)
        if res:
            return res
        else:
            response['text']="Something wrong. try again"
